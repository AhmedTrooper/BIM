...
export const CustomPresetCreator: React.FC<CustomPresetCreatorProps> = ({
  ...
  const parseSize = (config: SizeConfig): { width: number; height: number; name: string } | null => {
    try {
      if (config.type === 'square') {
        const size = parseInt(config.value);
  ...
  const handleGenerate = () => {
    ...
    for (const sizeConfig of sizes) {
      const parsed = parseSize(sizeConfig);
      if (parsed) {
        variants.push({
    ...
    if (variants.length > 0) {
      ...
      alert('Please add at least one valid size configuration');
    }
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-4 sm:p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl sm:text-2xl font-bold">Create Custom Preset</h2>
            <button
              onClick={onClose}
          ...
          <div className="mb-6">
            ...
            <div className="space-y-3">
              {sizes.map((size) => (
                <div key={size.id} className="flex flex-col sm:flex-row gap-2 p-3 border rounded-md dark:border-gray-600">
                  <select
                    value={size.type}
                    onChange={(e) => updateSize(size.id, 'type', e.target.value)}
                  ...
                  <input
                    ...
                    placeholder={
                      size.type === 'square' ? '512' :
                      size.type === 'dimension' ? '1920x1080' :
                      '16:9@1920'
                    }
            ...
            <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md text-sm">
              ...
              <ul className="space-y-1 text-xs">
                <li><strong>Square:</strong> 16, 32, 64, 512 → creates 16x16, 32x32, etc.</li>
                <li><strong>Dimension:</strong> 1920x1080, 800x600 → creates exact dimensions</li>
                <li><strong>Aspect Ratio:</strong> 16:9@1920 → creates 1920x1080 (16:9 ratio at 1920px width)</li>
              </ul>
            </div>
          </div>
          ...
          <div className="mb-6 p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
            <h3 className="text-sm font-semibold mb-2">Preview ({sizes.filter(s => parseSize(s)).length} variants):</h3>
            <div className="space-y-1 text-xs max-h-40 overflow-y-auto">
              {sizes.map((size) => {
                const parsed = parseSize(size);
                return parsed ? (
                  <div key={size.id} className="flex justify-between">
                    <span className="font-mono">{parsed.name}.{format}</span>
                    <span className="text-gray-500">{parsed.width}x{parsed.height}</span>
                  </div>
                ) : null;
              })}
            </div>
